...
def coord2str(coord):
    '''Converts a coordinate object of form {'lat': lat, 'lon': lon} to a string
    of the format "lat,lon"
    '''
    return "{0:.7f},{1:.7f}".format(coord['lat'], coord['lon'])

def str2coord(string):
    '''Converts a a string of the format "lat,lon" to a coordinate object of 
    form: {'lat': lat, 'lon': lon}
    '''
    lat, lon = string.split(",")[0], string.split(",")[1]
...
class Database:
    ...
    def getAvgTravelTime(self, start, end):
        ...
        try:
            ...
            self.cursor.execute("select int_id from intersections where lat = {0:.7f} and lon = {1:.7f};".format(lat, lon))
            log.debug("select int_id from intersections where lat = {0:.7f} and lon = {1:.7f};".format(lat, lon))
            ...
            self.cursor.execute("select sec_to_time(avg(time_to_sec(duration))) as avg_time from times where int_id1 in (%s) and int_id2 in (%s);" % (set1, set2))
            # ... "and hour(time) between 5 and 6"
    ...
    def getTravelTime(self, node1, node2):
        ...
        and speed limit. node1 and node2 are coordinate objects'''
        # Check the average of the previously recorded times
        avg = self.getAvgTravelTime(node1, node2)
        if avg is not None:
            log.debug("Travel time between %s and %s is %f" % (coord2str(node1), coord2str(node2), avg))
            return avg
        ...
        log.debug("Travel time between %s and %s is %f seconds." % (coord2str(node1), coord2str(node2), dist/speed))
        return dist / speed 
    ...
    def setTravelTime(self, lat_start, lon_start, lat_end, lon_end, duration):
        ...
        self.cursor.execute("insert into times (int_id1, int_id2, time, duration) select a.int_id, b.int_id, NULL, {0} from (select int_id from intersections where lat = {1:.7f} and lon = {2:.7f} limit 1) as a, (select int_id from intersections where lat = {3:.7f} and lon = {4:.7f} limit 1) as b".format(duration, lat_start, lon_start, lat_end, lon_end))
    ...
    def getNeighbors(self, node):
        ...
        # All neighbor nodes
        self.cursor.execute("select int2lat as lat, int2lon as lon from segments where int1lat = {0:.7f} and int1lon = {1:.7f} union select int1lat as lat, int1lon as lon from segments where int2lat = {0:.7f} and int2lon = {1:.7f} and oneway = 0;".format(lat, lon))
        # Just intersections
